...
# 电影搜索
movie_name = st.text_input("请输入电影名称（如：肖申克的救赎）", value="", placeholder="输入电影名进行搜索...")
...
if st.session_state.analyzed_movie_id and st.session_state.analyzed_movie_id == movie_id:
    ...
    with col3:
        st.metric("加权好评率", f"{pos_weighted_rate:.1f}%", help="考虑星级、点赞数和情感强度的加权好评率")
    with col4:
    ...
    with col_left:
        ...
        show_weighted = st.checkbox("显示加权分布", value=False, help="加权分布考虑了评论的星级、点赞数和情感强度")
    ...
    with col_right:
        ...
        use_ai_filter = st.checkbox("使用AI智能筛选关键词", value=True, help="AI筛选可以过滤无意义词汇，保留真正的关键词")

        word_freq = get_word_frequency(df, top_n=100)
        if word_freq:
            from analysis.nlp_processor import smart_filter_keywords
            filtered_keywords = smart_filter_keywords(word_freq, top_n=50, use_ai=use_ai_filter)
            ...
            if not word_dict:
                ...
                try:
                    ...
                    st.caption(f"筛选方式: {filter_method} | 关键词数量: {len(word_dict)}")
                except Exception as e:
                    st.error(f"词云生成失败: {e}")
    ...
    if sort_by == "按权重排序（推荐）":
        ...
        df_display['comment_time'] = pd.to_datetime(df_display['comment_time'], errors='coerce')
        df_display = df_display.sort_values('comment_time', ascending=False)

    display_df = df_display[["user_name", "star_rating", "comment_text", "vote_count", "weight", "comment_time"]].head(50)
    ...
    movie_info = db.get_movie_info(movie_id)
    genres = movie_info.get('genres', '').split(',') if movie_info and movie_info.get('genres') else []
    ...
    if not genres:
        ...
        if not similar_movies:
            st.info("数据库中暂无同类电影数据。分析其他同类电影后，这里将自动显示对比图表。")
        else:
            ...
            compare_data = [{
                ...
                "好评率": round(current_dist["正面"] / current_total * 100, 1) if current_total else 0,
            ...
            for m in similar_movies:
                ...
                compare_data.append({
                    ...
                    "movie_id": m['movie_id'],
                    "豆瓣评分": m['avg_rating'],
                    "好评率": round(m_dist["正面"] / m_total * 100, 1) if m_total else 0,
            ...
            bar = (
                ...
                .add_yaxis("加权好评率 (%)", compare_df["加权好评率"].tolist(), color="#409EFF")
                .set_global_opts(
